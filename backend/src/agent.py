...
try:
    from escalation import create_escalation_request, init_escalation_table
except ImportError:
    from src.escalation import create_escalation_request, init_escalation_table
...
class Assistant(Agent):
    ...
    async def create_escalation(
        ...
        """Create a human-escalation request when you cannot help the farmer further.

        ONLY call this tool after the farmer has EXPLICITLY agreed to share their
        ...
        Args:
            ...
            caller_name     : Farmer's first name only (no phone / account numbers).
            caller_lang     : Language spoken (e.g. 'Malayalam', 'English', 'Manglish').
            follow_up_pref  : How the farmer wants to be contacted:
                              'voice_call', 'sms', or 'whatsapp'.
            already_checked : What you (the agent) already tried (e.g. tools called,
                              data returned).
            crop            : Crop name if relevant (e.g. 'rubber', 'paddy').
            district        : District name if relevant (e.g. 'Kottayam').
            urgency         : 'low', 'medium', or 'high'.
        """
        logger.info(
            f"Creating escalation: reason='{reason}', caller='{caller_name}', "
            f"crop='{crop}', district='{district}', urgency='{urgency}'"
        )
        try:
            ...
            logger.error(f"Escalation creation failed: {exc}")
            return json.dumps(
